...
/*
        ...
        * 자료형
        ...
        > VARCHAR2 : 최대 4000바이트까지 지정 가능. 가변 길이(담긴 값에 따라서 공간의 크기가 맞춰짐)
...
-- 만약 컬럼명에 오타가 발생했다면?
-- 다시 만들면 될까?? ㄴㄴ 삭제하고 다시 만들어야 함..ㅠㅠ
-- DROP TABLE 테이블명;
...
-- INSERT INTO MEMBER VALUES (1, 'user01', 'pass01', '홍길동'); -- 다 입력을 안 하면 에러남
...
/*
        ...
        ** NOT NULL 제약조건은 오로지 컬럼레벨방식밖에 안됨 **
*/
...
-- ORA-00001: unique constraint (DDL.SYS_C007137) violated
...
-- > 성별에 유효한 값이 아닌 게 들어와도 잘 INSERT가 된다. => 이러면 안됨..
...
-- ORA-02290: check constraint (DDL.SYS_C007152) violated
-- CHECK 제약조건에 위배됐기 때문에 오류 발생

INSERT INTO MEM_CHECK VALUES (2, 'USER02', 'PASS02', '이강인', 'ㄴ', NULL, NULL);
-- 만일 GENDER 컬럼에 데이터 값을 넣고자 한다면 CHECK 제약조건에 만족하는 값을 넣어야 됨
...
INSERT INTO MEM_PRI2 
VALUES (NULL, 'USER01', 'PASS02', '이승우', NULL, NULL, NULL);
...
INSERT INTO TB_LIKE VALUES (1, '감자', SYSDATE); -- 에러 발생!! 한번만 찜해야됨
...
INSERT INTO MEM
VALUES(3, 'USER03', 'PASS03', '이승우', '남', NULL, NULL, 40);
-- ORA-02291: integrity constraint (DDL.SYS_C007181) violated - parent key not found
...
WHERE GRADE_CODE = 10;
-- ORA-02292: integrity constraint (DDL.SYS_C007181) violated - child record found
--> 자식테이블 (MEM)에 10이라는 값을 사용하고 있기 때문에 삭제가 안됨!

DELETE FROM MEM_GRADE
WHERE GRADE_CODE = 30;
--> 자식테이블 (MEM)에 30이라는 값을 사용하고 있지 않기 때문에 삭제가 가능!!
...
/*
        ...
        * 삭제옵션 : 부모테이블의 데이터 삭제시 그 데이터를 사용하고 있는 자식 테이블의 값을
                    어떻게 처리할건지 
        - ON DELETE RESTRICTED (기본값) : 삭제제한옵션으로, 자식데이터로 쓰이는 부모데이터는 삭제가 아예 안되게끔
...
-- INSERT INTO MEMBER VALUES (4, '김민재', DEFAULT, DEFAULT, DEFAULT);

-- INSERT INTO 테이블명 (컬럼명, 컬럼명) VALUES (값1, 값2);
-- NOT NULL인 건 꼭 써야함!!

INSERT INTO MEMBER(MEM_NO, MEM_NAME) VALUES (4, '김민재');
...
AS SELECT EMP_ID, EMP_NAME, SALARY, BONUS
    ...
    WHERE 1 = 0; -- 무조건 FALSE인 조건 : 구조만을 복사하고자 할 때 쓰이는 구문 (데이터 값은 필요없을 때)
...
-- alias : 별칭
--> 서브쿼리 SELECT 절에 산술식 또는 함수식이 기술된 경우 반드시 별칭을 지정해야됨!!
...
-- 서브쿼리를 이용해서 복제한 테이블 NN 제약조건 빼고 복제가 안됨
