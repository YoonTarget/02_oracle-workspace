/*
    ...
    * 트랜젝션(TRANSACTION)
    ...
    - 데이터의 변경사항(DML 추가, 수정, 삭제)들을 하나의 트랜젝션에 묶어서 처리
      ...
      COMMIT을 해야만 실제 DB에 반영된다고 생각하면 됨!!
      ...
      - SAVEPOINT 포인트명; 진행 : 현재 이 시점에서 해당 포인트명으로 임시저장점을 정의해두는 것
                                 ROLLBACK 진행시 전체 변경사항들을 다 삭제하는 게 아니라 일부만 롤백 가능
...
-- DDL문 (CREATE, DROP, ALTER)을 수행하는 순간 기존에 트랜젝션에 있던 변경사항들을
-- 무조건 COMMIT(실제 DB에 반영)
-- 즉, DDL문 수행 전 변경사항이 있었다면 정확히 FIX(COMMIT OR ROLLBACK)하고 하자!!
