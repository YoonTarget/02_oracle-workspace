...
FROM (SELECT EMP_ID, EMP_NAME, DEPT_TITLE, SALARY, NATIONAL_NAME
      ...
      JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
      JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
      JOIN NATIONAL USING (NATIONAL_CODE)); -- 이건 인라인뷰 // 위는 뷰!!
...
-- 뷰를 통해 조작하더라도 실제 데이터가 담겨있는 베이스 테이블에 반영됨
...
/*
    * 단, DML 명령어로 조작이 불가능한 경우가 더 많음!!
    ...
    2) 뷰에 정의되어 있지 않은 컬럼 중에 베이스 테이블 상에 NOT NULL 제약조건이 지정되어 있는 경우 => 어떤 건 되고, 어떤 건 안됨,,
    3) 산술연산식 또는 함수식으로 정의되어 있는 경우
    4) 그룹함수나 GROUP BY절이 포함된 경우
    5) DISTINCT 구문이 포함된 경우
    6) JOIN을 이용해서 여러 테이블을 연결시켜놓은 경우
    => 뷰는 조회하려고 만든거라서 DML은 하지 말자!!
*/
...
DELETE FROM VW_JOB
WHERE JOB_NAME = '사원';
-- ORA-02292: integrity constraint (KH.SYS_C007217) violated - child record found

-- 3) 산술연산식 또는 함수식으로 정의되어 있는 경우
CREATE OR REPLACE VIEW VW_EMP_SAL (사번, 사원명, 월급, 연봉)
...
/*
    * VIEW 옵션
    ...
    2) FORCE | NOFORCE
        > FORCE : 서브쿼리에 기술된 테이블이 존재하지 않아도 뷰가 생성되게 하는 구문
        > NOFORCE : 서브쿼리에 기술된 테이블이 존재하는 테이블이어야만 뷰가 생성되게 하는 구문 (생략시 기본값)
...
-- 3) WITH CHECK OPTION : 서브쿼리에 기술된 조건에 부합하지 않은 값으로 수정시 오류 발생
--    WITH CHECK OPTION 안 주고
...
INSERT INTO VW_EMP VALUES (100, '차은우', 0.2);
