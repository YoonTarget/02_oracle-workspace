/*
    ...
    SQL 문장 내에서 변수의 정의, 조건처리(IF), 반복처리(LOOP, FOR, WHILE) 등을 지원하여 SQL의 단점 보완
...
DECLARE
    ...
    TEAM VARCHAR2(10);
BEGIN
    -- 사용자가 입력한 사번을 가지고 있는 사원의 사번, 이름, 부서명, 근무국가 코드 조회 후 각 변수에 대입
    ...
    -- 사번, 이름, 부서, 소속(국내팀 OR 해외팀)에 대해 출력
    DBMS_OUTPUT.PUT_LINE('사번 : ' || EID);
    DBMS_OUTPUT.PUT_LINE('이름 : ' || ENAME);
...
--------------------------------------------------------------------------------
-- 3) IF 조건식1 THEN 실행내용1 ELSIF 조건식2 THEN 실행내용... ELSE 실행내용 END IF; (IF - ELSE IF - ELSE 문)
...
--------------------------------------------------------------------------------
-- 4) CASE 비교대상자 WHEN 동등비교할 값1 THEN 결과값1 WHEN 비교값2 THEN 결과값2 ... ELSE 결과값 END;
DECLARE
...
-- 보너스가 있으면 보너스 포함 연봉
-- 출력 예시
-- 급여, 이름, 연봉(\999,999,999) (8000000 선동일 \124,800,800)
DECLARE
    SAL EMPLOYEE.SALARY%TYPE;
    ENAME EMPLOYEE.EMP_NAME%TYPE;
    YEAR_SAL NUMBER;
BEGIN
    SELECT SALARY, EMP_NAME, TRIM(TO_CHAR((SALARY + (SALARY * NVL(BONUS, 0))) * 12, 'L999,999,999'))
    INTO SAL, ENAME, YEAR_SAL
    FROM EMPLOYEE
    WHERE EMP_NAME = '&사원';
    
    DBMS_OUTPUT.PUT_LINE(SAL || ' ' || ENAME || ' ' || YEAR_SAL);
END;
/
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE(EMP.SALARY || ' ' || EMP.EMP_NAME || ' ' || TRIM(TO_CHAR(SALARY, 'L999,999,999')));
END;
/
...
BEGIN
    ...
    INTO EID, ENAME
    FROM EMPLOYEE
    WHERE MANAGER_ID = &사수사번; -- 211 장쯔위 200 투매니 202 전혀없음
