...
/*
    ...
    [CYCLE | NOCYCLE]             -- 값 순환여부 지정 (기본값 NOCYCLE) => 최대값 찍고 처음으로 다시 돌아와서 시작할 수 있게 함   
    [NOCACHE | CACHE 바이트크기]    -- 캐쉬메모리 할당 (기본값 CACHE 20)
    ...
    * 캐시메모리 : 임시공간
                ...
                번호가 일정하게 부여 안 될 수가 있으니 확인을 잘 해야함!!
...
-- NEXTVAL를 단 한 번도 수행하지 않는 이상 CURRVAL 할 수 없음!!
...
SELECT SEQ_EMPNO.NEXTVAL FROM DUAL; -- 지정한 MAXVALUE 값을 초과했기 때문에 오류 발생!! (실패)
SELECT SEQ_EMPNO.CURRVAL FROM DUAL;
