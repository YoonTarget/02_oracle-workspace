...
-- 사수가 없는 사원(MANAGER_ID 값이 NULL인)들의 사원명, 사수 사번, 부서코드 조회
...
-- 5. 보너스 포함 연봉이 NULL이 아니고 이름에 '하'가 포함되어 있는 사원들의 (사번, 사원명, 급여, 보너스 포함 연봉) 조회
SELECT EMP_ID, EMP_NAME, SALARY, (SALARY + (SALARY * BONUS)) * 12 AS "ANNUAL SALARY INCLUDED BONUS"
FROM EMPLOYEE
...
/*
        < ORDER BY 절 >
        가장 마지막 줄에 작성! 뿐만 아니라 실행순서 또한 가장 마지막에 실행
        ...
        - NULLS LAST : 정렬하고자 하는 컬럼값에 NULL이 있을 경우 해당 데이터를 맨 뒤 배치 (생략시 ASC일 때의 기본값)
*/
...
ORDER BY BONUS DESC, SALARY ASC;        -- 정렬기준 여러개 제시 가능 (첫번째 기준의 컬럼값이 동일할 경우, 두번째 기준의 컬럼을 가지고 정렬)
