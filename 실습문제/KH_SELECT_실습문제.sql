...
SELECT EMP_NAME, (SALARY * 12) AS "연봉", (SALARY + SALARY * NVL(BONUS, 0)) * 12 AS "총수령액(보너스 포함)", (SALARY + SALARY * NVL(BONUS, 0)) * 12 - (SALARY * 12 * 0.03) AS "실수령액"
FROM EMPLOYEE;
...
SELECT EMP_NAME, SUBSTR (EMP_NO, 1, 2) || '년' AS "생년", SUBSTR (EMP_NO, 3, 2) || '월' AS "생월", SUBSTR (EMP_NO, 5, 2) || '일' AS "생일"
FROM EMPLOYEE;
...
SELECT EMP_NAME, ABS (FLOOR ( HIRE_DATE - SYSDATE)) AS "근무일수 1", FLOOR (SYSDATE - HIRE_DATE) AS "근무일수 2"
FROM EMPLOYEE;
...
SELECT EMP_NAME, TO_CHAR (SALARY, 'L9,999,999') || '원' AS "급여"
FROM EMPLOYEE;

-- 21. EMPLOYEE테이블에서 직원 명, 부서코드, 생년월일, 나이(만) 조회 (단, 생년월일은 주민번호에서 추출해서 00년 00월 00일로 출력되게 하며 나이는 주민번호에서 출력해서 날짜데이터로 변환한 다음 계산)
SELECT EMP_NAME, DEPT_CODE, TO_CHAR (TO_DATE (SUBSTR (EMP_NO, 1, 6)), 'YY"년" MM"월" DD"일"') AS "생년월일", FLOOR ((SYSDATE - TO_DATE (SUBSTR (EMP_NO, 1, 6))) / 365) || '세' AS "나이(만)"
FROM EMPLOYEE;

-- 22. EMPLOYEE테이블에서 부서코드가 D5, D6, D9인 사원만 조회하되 D5면 총무부, D6면 기획부, D9면 영업부로 처리 (단, 부서코드 오름차순으로 정렬)
SELECT EMP_ID, EMP_NAME, EMP_NO, EMAIL, PHONE, JOB_CODE, SAL_LEVEL, SALARY, BONUS, MANAGER_ID, HIRE_DATE, ENT_DATE, ENT_YN, DEPT_CODE,
...
SELECT EMP_NAME, SUBSTR (EMP_NO, 1, 6) AS "주민번호 앞자리", SUBSTR (EMP_NO, 8, 14) AS "주민번호 뒷자리", SUBSTR (EMP_NO, 1, 6) + SUBSTR (EMP_NO, 8, 14) AS "앞자리와 뒷자리의 합"
FROM EMPLOYEE
WHERE EMP_ID = 201;

-- 24. EMPLOYEE테이블에서 부서코드가 D5인 직원의 보너스 포함 연봉 합 조회
SELECT SUM (SALARY + (SALARY * NVL(BONUS, 0)) * 12) || '원' AS "연봉 합"
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5';

-- 25. EMPLOYEE테이블에서 직원들의 입사일로부터 년도만 가지고 각 년도별 입사 인원수 조회 전체 직원 수, 2001년, 2002년, 2003년, 2004년
SELECT COUNT(*) AS "전체 직원 수", COUNT(DECODE(SUBSTR(HIRE_DATE, 1, 2), '01', 1)) AS "2001년", 
COUNT(DECODE(SUBSTR(HIRE_DATE, 1, 2), '02', 1)) AS "2002년", COUNT(DECODE(SUBSTR(HIRE_DATE, 1, 2), '03', 1)) AS "2003년", COUNT(DECODE(SUBSTR(HIRE_DATE, 1, 2), '04', 1)) AS "2004년"
FROM EMPLOYEE;
