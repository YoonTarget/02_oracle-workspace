...
SELECT EMP_NAME, SALARY, 실수령액, HIRE_DATE
FROM (SELECT EMP_NAME, (SALARY * 12) AS "연봉", (SALARY + SALARY * NVL(BONUS, 0)) * 12 AS "총수령액(보너스 포함)", (SALARY + SALARY * NVL(BONUS, 0)) * 12 - (SALARY * 12 * 0.03) AS "실수령액",SALARY , HIRE_DATE
        FROM EMPLOYEE)
WHERE 실수령액 >= 50000000;
...
SELECT EMP_NAME, DEPT_CODE, TO_CHAR (TO_DATE (SUBSTR (EMP_NO, 1, 6)), 'YY"년" MM"월" DD"일"') AS "생년월일", 
        FLOOR((SYSDATE - TO_DATE(SUBSTR(EMP_NO, 1, 6))) / 365) || '세' AS "나이(만)"
FROM EMPLOYEE;
...
SELECT EMP_NAME, SUBSTR (EMP_NO, 1, 6) AS "주민번호 앞자리", SUBSTR (EMP_NO, 8) AS "주민번호 뒷자리", SUBSTR (EMP_NO, 1, 6) + SUBSTR (EMP_NO, 8) AS "앞자리와 뒷자리의 합"
FROM EMPLOYEE
WHERE EMP_ID = 201;
