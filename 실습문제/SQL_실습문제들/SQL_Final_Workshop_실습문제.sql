...
-- “작가(명)”으로 표시되도록 할 것)
SELECT COUNT(DISTINCT WRITER_NM) AS "작가(명)"
FROM TB_WRITER
...
INSERT INTO TB_BOOK_TRANSLATOR (SELECT * FROM TB_BOOK_AUTHOR
...
DELETE FROM VW_BOOK_TRANSLATOR
WHERE ISSUE_DATE = '07/10/31';
...
SELECT W.BOOK_NM AS "도서명", W.WRITER_NM AS "저자", T.WRITER_NM AS "역자"
FROM (SELECT BOOK_NM, WRITER_NM
        FROM TB_WRITER
        JOIN TB_BOOK_AUTHOR USING (WRITER_NO)
        JOIN TB_BOOK USING (BOOK_NO)) W,
        (SELECT BOOK_NM, WRITER_NM
        FROM TB_BOOK_TRANSLATOR
        JOIN TB_WRITER USING (WRITER_NO)
        JOIN TB_BOOK USING (BOOK_NO)
        WHERE BOOK_NM = '아타트롤') T
WHERE W.BOOK_NM = '아타트롤';
...
SELECT BOOK_NM AS "도서명", STOCK_QTY AS "재고수량", PRICE AS "가격(Org)", PRICE * 0.8 AS "가격(New)",
        ISSUE_DATE
FROM TB_BOOK
WHERE (SYSDATE - TO_DATE(ISSUE_DATE)) / 365 > 31
AND STOCK_QTY >= 90
ORDER BY 2 DESC, 4 DESC, 1;

SELECT COUNT(DISTINCT BOOK_NM)
FROM TB_BOOK;
